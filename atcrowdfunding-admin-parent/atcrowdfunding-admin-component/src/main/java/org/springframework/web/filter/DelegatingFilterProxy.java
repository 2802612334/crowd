    ...
    protected void initFilterBean() throws ServletException {
        synchronized (this.delegateMonitor) {
            if (this.delegate == null) {
                ...
                /*
                源码中：获取的Spring的IOC容器。因为SpringSecurity的核心配置被SpringMVC扫描，所以SpringSecurity的Filter默认加载到SpringMVC的IOC容器中。
                默认的加载顺序是
                    首先加载Spring的ContextLoaderListener，Spring的容器。
                    接下来加载DelegatingFilterProxy的init初始化方法，
                    最后加载SpringMVC的IOC容器，
                    在这里不获取SpringMVC的IOC容器，是因为DelegatingFilterProxy优先于SpringMVC的IOC容器加载，在初始化方法这里获取MVC容器会获取不到。
                    必须要等到SpringMVC的IOC容器加载完之后，才可以获取。
                    该类的doFilter方法中获取SpringMVC的IOC容器。
        ...
        if (delegateToUse == null) {
            synchronized (this.delegateMonitor) {
                ...
                if (delegateToUse == null) {
                    ...
                    // 获取SpringMVC的IOC容器，替换Spring的IOC容器
                    ServletContext servletContext = this.getServletContext();
                    String mvcIOCName = FrameworkServlet.SERVLET_CONTEXT_PREFIX + "dispatcherServlet";
                    WebApplicationContext wac = (WebApplicationContext) servletContext.getAttribute(mvcIOCName);
